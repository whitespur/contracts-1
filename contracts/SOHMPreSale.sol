...
contract SOHMPreSale is Ownable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    address public alphaSOHM;
    ...
    mapping(address => bool) public boughtSOHM;
    ...
    function initialize(address payable _DAOAddress,
        ...
        uint _toTalAmount,
        uint _salePrice,
        uint _saleLength) external onlyOwner() returns (bool) {
        require(saleStarted == false, 'Already initialized');

        alphaSOHM = _alphaSOHM;
    ...
    function purchaseaSOHM() external payable returns (bool) {
    ...
    function sendRemainingaOHM() external onlyOwner() returns (bool) {
        ...
        IERC20(alphaSOHM).safeTransfer(msg.sender, IERC20(alphaSOHM).balanceOf(address(this)));
        return true;
    }
