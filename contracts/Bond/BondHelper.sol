...
interface IReward {
    ...
    function lastEpochId_() external view returns (uint256);
}
...
contract DepositHelper is Ownable {
    ...
    address public SOHM; // SameUsd token
    ...
    constructor (address _deposit, ISwapV2Router _router, address _principle, address _SOHM, address _WBNB, ITreasury _treasury) {
    ...
    function depositHelper(
        ...
        if (_tokenAddress == principle) {
            ...
            return IDepositor(deposit).deposit(_amount, _maxPrice, msg.sender);
        } else {
            ...
            SOHM.safeApprove(address(router), uint256(- 1));

            calAndSwap();
            (,, uint256 moreLPAmount) = router.addLiquidity(WBNB, SOHM, WBNB.myBalance(), SOHM.myBalance(), 0, 0, address(this), block.timestamp);
            principle.safeApprove(address(deposit), moreLPAmount);
            if (WBNB.myBalance() > 0) {
                WBNB.safeTransfer(msg.sender, WBNB.myBalance());
            }
            if (SOHM.myBalance() > 0) {
                SOHM.safeTransfer(msg.sender, SOHM.myBalance());
            }
            return IDepositor(deposit).deposit(moreLPAmount, _maxPrice, msg.sender);
        }
    }

    function depositValue(uint256 _amount) public view returns (uint256 value_) {
        ISwapV2Pair lpToken = ISwapV2Pair(factory.getPair(WBNB, SOHM));
        (uint256 token0Reserve, uint256 token1Reserve,) = lpToken.getReserves();
        (uint256 debtReserve, uint256 relativeReserve) = WBNB ==
        ...
        if (swapAmt > 0) {
            ...
            (path[0], path[1]) = isReversed ? (SOHM, WBNB) : (WBNB, SOHM);
            uint[] memory amounts = router.getAmountsOut(swapAmt, path);
            (uint256 amount0, uint256 amount1) = WBNB == lpToken.token0() ? (_amount.sub(swapAmt), amounts[1]) : (amounts[1], _amount.sub(swapAmt));
    ...
    function calAndSwap() internal {
        ...
        (uint256 debtReserve, uint256 relativeReserve) = WBNB ==
        lpToken.token0() ? (token0Reserve, token1Reserve) : (token1Reserve, token0Reserve);
        (uint256 swapAmt, bool isReversed) = optimalDeposit(WBNB.myBalance(), SOHM.myBalance(),
            debtReserve, relativeReserve);
    ...
    /**
    *  @notice 每bnb的bond预计多少奖励 
    */
    function perBnbReward() view public returns (uint256){
        uint256 valForBond = depositValue(1e18);
        uint256 _lastEpochId = IReward(reward).lastEpochId_();
        (,,uint256 _totalBond,uint256 _lastReward,) = IReward(reward).epochsInfo(_lastEpochId);
        uint256 _reward = _lastReward.mul(valForBond).div(_totalBond.add(valForBond));
        return _reward;
    }
